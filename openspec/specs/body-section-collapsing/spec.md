# body-section-collapsing Specification

## Purpose
Navigable, gutter-marked folding of nested JSON objects/arrays in the read-only request and response body views, including the line cursor, fold-state model, and the collapse/expand toggle. Folding is view-only and never mutates stored bodies.
